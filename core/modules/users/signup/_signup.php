...
class _Signup extends Users{
	...
	function checkLogin($login = false) {
		...
		if ($login && preg_match ( "'^[a-z0-9_-]{3,}$'i", $login )) {
			$cn = $this->db->getOne ( "SELECT COUNT(*) FROM {$this->table} WHERE login='{$login}'");
			if ($cn > 0)
			$stat = 2;
		} else
		$stat = 1;
	...
	function save($id = null, $message = false) {
		...
		$send = new sendmail ();
		$send->addHtml ( 'Вы зарегистрировались на ' . DOMAIN_CLEAR . ' <br/>
			Для активации перейдите по ссылке <br/><a href="' . DOMAIN . 'signup/?hash=' . $_POST ['hash'] . '">' . DOMAIN . 'signup/?hash=' . $_POST ['hash'] . '</a>' );
		$send->send ( $_POST ['email'], 'Активация аккаунта на сайте ' . DOMAIN_CLEAR, '', false, false );
