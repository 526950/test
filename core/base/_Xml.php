...
class _Xml {
	private static $dom, $root, $header_xml = true, $ndom;
	...
	public static function transform($root_tag_name = false, $xsl_filename, $xml = null, $debug = false) {
		...
		$xml_filename = (isset($_GET['ajaxModule'])) ? 'index_ajax' : substr(basename($_SERVER['PHP_SELF']), 0, -4);
	...
	private static function protect_xml($xml = null) {
		if (!is_null($xml)) {
			...
			if (!(self::$dom instanceof DOMDocument))
				Error::getErrors('self::$dom not instance of DOMDocument');
	...
	public static function debug($xml = null, $filename = null) {
		...
		if (is_string($filename)) {
			$xml->save(TEMP . basename($filename) . '.xml');
		} else {
			if (self::$header_xml) {
	...
	static function get_root_tag($xpath_query, &$tag_name) {
		...
		if (is_null($root_tag))
			Error::getErrors("Xpath - {$xpath_query} не найден");
		else
			return $root_tag;
	}
	 ...
	 *
	 * @param string $root_tag_name название корневого тэга для DOM если он еще не создан (если внутренний DOM существует то создать ребенка у корневого тэга и вренуть ссылку на него) (если на вход ничего не продано а DOM существует, то просто вернуть линк на корневой тэг)
	 * @return object DOMDocument
	 */
	 ...
	 * @param unknown_type $default_name значение по умолчанию, на которое будет заменено название тэга если он не пройдет проверку, если же значение по умолчанию также не проходит проверку то тэг будет называться 'node'
	 */
	private static function protect_tag(&$tag_name, $default_name = false) {
		if (!is_string($tag_name) || !mb_eregi("^[А-Яа-яa-z_]{1}[А-Яа-яa-z0-9_-]*$", $tag_name)) {
			(!is_string($default_name) || !mb_eregi("^[а-яa-z_]{1}[А-Яа-яa-z0-9_-]*$", $default_name)) ? $tag_name = 'item' : $tag_name = $default_name;
		}
	}
	...
	static function from_array($xpath_query, $array, $tag_name = false, $item = 'item', $keys_as_params = false, $cut_empty = false) {
		($xpath_query instanceof DOMElement) ? $root_tag = $xpath_query : $root_tag = self::get_root_tag($xpath_query, $tag_name);
		
		$dom = &self::$dom;
		$keys_as_params_ = $keys_as_params;
		$params = '';
		...
		if ($keys_as_params) {
			if (is_bool($keys_as_params)) {
				$keys_as_params = (isset($array[0])) ? array_keys($array[0]) : array_keys($array);
			}
			...
			foreach ( $tmp_params as $key => $as_param ) {
				...
				if (array_key_exists($as_param, $array) && !is_array($array[$as_param])) {
					$protect_param = $as_param;
					self::protect_tag($protect_param, 'atr' . $key);
					$root_tag->setAttribute($protect_param, $array[$as_param]);
	...
	public static function from_db($xpath_query, $query, $params = array(), $tag_name = false, $item = 'item', $keys_as_params = false) {
		...
		$array = call_user_func_array(array ($db,'getAll'), $params);
		
		if ($array)
			self::from_array($xpath_query, $array, $tag_name, $item, $keys_as_params);
		return $array;
	}

	static function format($xml = false) {
		if ($xml) {
			$indent = -1;
			$xml = str_replace(' "', '"', mb_ereg_replace("'(?<=>)\s+(?=<)'", '', mb_ereg_replace("'\s+'", ' ', $xml)));

			function iteration($matches) {
				global $indent;
				$m = substr($matches[2], -1);
				if ($m != "/" && $matches[1] != '/' && $matches[4] == "</") {
					return "<{$matches[1]}{$matches[2]}{$matches[3]}";
				} else {
					$indent += (($m == "/" && $matches[4] == "</") || ($matches[1] == '/' && $matches[4] == "</")) ? -1 : (($m == '/') ? 0 : (($matches[1] == '/') ? 0 : 1));
					return "<{$matches[1]}{$matches[2]}{$matches[3]}\n" . str_repeat("\t", $indent);
				}
			}
			$xml = preg_replace_callback("'<(/?)([^>]+)(>)\s*(?=(</?))'", "iteration", $xml);
		}
		return $xml;
	}
}